...
gage_data <- read.csv("TAPR_reach_and_gage_locations.csv")
...
discharge <- discharge %>%
  filter(site_no %in% c("07180400", "07182250"))
...
for (station in unique(discharge$site_no)) {
  ...
  for (water_year in unique(station_data$wy_val)) {
    ...
    days_over_7xmedian <- sum(water_year_data$Flow >= (7*long_median), na.rm = TRUE)
    ...
    result_df <- rbind(result_df, data.frame(
      ...
      days_over_7xmedian = days_over_7xmedian,
      high_flow_duration = round(mean_duration),
      fall_rate = format(round(mean_rate_change,2)),
...
write.csv(d, "TAPR_fish_data.csv")
