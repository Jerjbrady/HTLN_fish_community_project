...
reach_coords <- read.csv("TAPR_coordinates.csv")


TAPR_gage_data <- gage_data %>%
   filter(ParkCode == "TAPR") %>%
  filter(GaugeSiteNo %in% c(7180400, 7182250))


reach_coords <- reach_coords %>%
  mutate(LocationID = gsub(".*FISH.", "", LocationID)) %>%
  filter(ParkCode == "TAPR") %>% drop_na()


# convert to spatial point

gages_sf <- st_as_sf(TAPR_gage_data, coords = c("Longitude", "Latitude"), crs = 4326)
...
nearest_gage_id <- TAPR_gage_data$GaugeSiteNo[nearest_gage_index]
...
write.csv(final_data, "TAPR_reach_and_gage_locations.csv")
